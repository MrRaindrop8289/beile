					...
					<form action="${pageContext.request.contextPath }/UpdateStudent" method="post">
						<tr >
							...
							<c:if test="${User.yonghu_jb > 1 }">
								<td rowspan="${Student.tsum }"><input type="text" name="phone" value="${Student.phone }" readonly="readonly" /></td>
							</c:if>
							...
							<c:forEach items= "${TclassList}" var="t" varStatus="status">
								<c:if test="${status.count == 1 }">
									...
									<c:if test="${User.yonghu_jb == 4 }">
										<c:if test="${t.is_gw_change == flase }">
											<td><button type="submit" name="tclassid" value="${t.mid }" class="btn btn-primary">试听课修改</button></td>
										</c:if>
										<c:if test="${t.is_gw_change == true }">
											<td>试听课修改</td>
										</c:if>
									</c:if>
									<c:if test="${User.yonghu_jb == 1 }">
									...
									<td rowspan="${Student.tsum }"><button type="submit" name="UpdateStudent" value="${Student.id }" class="btn btn-primary">提交修改</button>
									</tr>
								</c:if>
								<c:if test="${status.count > 1 }">
									<tr>
							...
							<c:if test="${TclassList == null }">
								<tr>
									<td><input type="text" name="ttime" value="" list="ManageList"/></td>
									<td><input type="text" name="result" value=""/></td>
									<td><button type="submit" name="newTclass" value="${s.id }" class="btn btn-primary">添加试听课</button>
									<td rowspan="${Student.tsum }"><button type="submit" name="UpdateStudent" value="${s.id }" class="btn btn-primary">提交修改</button>
								</tr>
							</c:if>
