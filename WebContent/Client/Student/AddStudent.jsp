					...
					<form action="${pageContext.request.contextPath }/AddStudent" method="post">
						<tr >
							<c:if test="${Student != null }">
								<td rowspan="${Student.tsum }"><input type="text" name="id" value="${Student.id }" readonly="readonly"/></td>
								<td rowspan="${Student.tsum }"><input type="text" name="shuju_ly" value="${Student.shuju_ly }" /></td>
								<td rowspan="${Student.tsum }"><input type="text" name="name" value="${Student.name }" style="width:100px"/></td>
								<td rowspan="${Student.tsum }"><input type="text" name="grade" value="${Student.grade }" style="width:100px"/></td>
								...
								<td rowspan="${Student.tsum }"><textarea rows="15" cols="90" name="content" value="${Student.content }" 
								...
								<c:if test="${Student.point == true }">
									<td rowspan="${Student.tsum }"><input type="checkbox" name="point" checked="checked" /></td>
								</c:if>
								<c:if test="${Student.point == false }">
									...
									<c:forEach items= "${TclassList}" var="t" varStatus="status">
										<c:if test="${status.count == 1 }">
											<td>${t.ttime }</td>
											<td>${t.result }</td>
											<td><button type="submit" name="tclassid" value="${t.mid }"  class="btn btn-primary">试听课修改</button>
											<td rowspan="${Student.tsum }"><button type="submit" name="AddStudent" value="${s.id }"  class="btn btn-primary">添加客户</button>
											</tr>
										</c:if>
										<c:if test="${status.count > 1 }">
											<tr>
									...
									<tr>
										<td><input type="text" name="ttime" value=""/></td>
										<td><input type="text" name="result" value=""/></td>
										<td><button type="submit" name="newTclass" value="newTclass" class="btn btn-primary">添加试听课</button>
									</tr>
							</c:if>
							
							<c:if test="${Student == null }">
								<td><input type="text" name="id" value="客户信息" readonly="readonly"/></td>
								<td><input type="text" name="shuju_ly"/></td>
								<td><input type="text" name="name" style="width:100px"/></td>
								<td><input type="text" name="grade" style="width:100px"/></td>
								<td><input type="text" name="phone"/></td>
								<td><input type="date" name="next_time"/></td>
								<td><textarea rows="15" cols="90" name="content" 
																style="border-bottom:#000000 1px solid;border-left::#000000 1px solid;
																border-right:#000000 1px solid;border-top:#000000 1px solid;font-size:9pt;
																height:100px;width:300px"></textarea></td>
								<td><input type="checkbox" name="point"/></td>
								<td><input type="text" name="ttime" value="" list="ManageList"/></td>
								<td><input type="text" name="result" value=""/></td>
								<td><button type="submit" name="newTclass" value="newTclass" class="btn btn-primary">添加试听课</button>
								<td><button type="submit" name="AddStudent" value="AddStudent" class="btn btn-primary">添加客户</button>
								</tr>
							</c:if>
